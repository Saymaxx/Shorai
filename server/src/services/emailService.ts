...
import { StoredLead } from '../db/database.js';
...
export class EmailService {
   ...
   */
  public static async sendAdminLeadNotification(lead: StoredLead): Promise<boolean> {
    const htmlContent = `
    ...
    try {
      await transporter.sendMail({
        from: `"Shorai Portal" <${ENV.SMTP_USER}>`,
        to: ENV.ADMIN_EMAIL,
   ...
   */
  public static async sendUserConfirmation(lead: StoredLead): Promise<boolean> {
    ...
    try {
      await transporter.sendMail({
        from: `"Shorai STEM Labs" <${ENV.SMTP_USER}>`,
        to: lead.email,
